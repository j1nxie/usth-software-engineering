 ...
 *           the exception
 *           contains a string explaining the problem. All instance methods
 *           modify the state of
 *           <code>this</code>.
 * 
 * 
...
public class Engine {
	 ...
	 *          throw <code>NotPossibleException</code> else creates NK and
	 *          initialises
	...
	/**
	 * A method to create a <code>Query</code> object containing the matching
	 * documents
	 * of a given keyword <code>w</code>
	 * 
	 * @param w a keyword to search
	 * @effects if <code>w</code> is not a word or <code>w</code> is an
	 *          uninteresting word
	 *          then throws <code>NotPossibleException</code>, else returns
	 *          a <code>Query</code> object containing the documents matching the
	 *          keyword
	 * @version 4.0
	 */
	public Query queryFirst(String w) throws NotPossibleException {
		...
		if (wt.lookup(w) == null) {
			throw new NotPossibleException(
					"Engine.queryFirst: the specified word is either not found in any documents or uninteresting: "
							+ w);
		}
	...
	/**
	 * A method to query the matching documents of an existing <code>Query</code>
	 * object for
	 ...
	 *          uninteresting word
	 *          then throws <code>NotPossibleException</code>, else returns
	 *          an updated <code>Query</code> object containing the documents
	...
	public Query queryMore(String w) throws NotPossibleException {
		...
		if (wt.lookup(w) == null) {
			throw new NotPossibleException(
					"Engine.queryMore: the specified word is either not found in any documents or uninteresting: " + w);
		}
	 ...
	 * @effects if <code>u</code> is not a URL for a web site containing documents
	 *          or
	 ...
	 *          <code>Query</code> object, else returns an updated object that
	 *          contains
	...
	public Query addDoc(Doc d) throws NotPossibleException {
		if (d == null) {
			throw new NotPossibleException("Engine.addDoc: d is null");
		}
		tt.addDoc(d);
