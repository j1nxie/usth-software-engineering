...
import a2_bi12_009.kengine.Doc;
import a2_bi12_009.kengine.DocCnt;
import a2_bi12_009.kengine.Engine;
import a2_bi12_009.kengine.Query;
import utils.DomainConstraint;
import utils.NotPossibleException;

/**
 * @overview
 *           Represents a worksheet program that enables a user to create
 *           student objects,
 *           display a report about them, and to search for objects of interest
 *           using keywords.
 * 
 * @attributes
...
public class ProgStudentMan {
	...
	/**
	 * @effects
	 *          Prompt user to input student type and then prompt to enter data to
	 *          create a student.
	...
	private void manageStudents() {
		...
		while (true) {
			...
			try {
				...
				switch (type) {
					case "s":
						c = createStudent(data);
						break;
					case "u":
						c = createUndergradStudent(data);
						break;
					case "p":
						c = createPostgradStudent(data);
						break;
					default:
	...
	/**
	 * @effects
	 *          prompt user to input the student type, which can be one one of
	 *          these:
	 *          "s" (student), "u" (undergraduate), "p" (postgraduate) or "c"
	 *          (cancel)
	...
	/**
	 * @effects
	 *          prompt user to input data suitable for the student typed
	 *          <tt>type</tt>
	 ...
	 *          prints "No students"
	 *          else
	 *          prints each object in this.objects one per line to the standard
	 *          output
	 */
	 ...
	 * @modifies System.out
	 * @effects
	 *          (1) invoke promptForKeywords() to prompt the user to enter one or
	 *          more keywords
	 ...
	 *          else
	 *          invoke displaySearchReport to display search result to the standard
	 *          output.
	...
	private void displaySearchReport(Query q) {
		...
		if (matches == null) {
			...
			for (int i = 0; i < col1Len + col2Len + 3; i++)
				hr.append("-");
	...
	/**
	 * @effects
	 *          initialise this to include an empty set of objects and an empty
	 *          engine
	 */
	...
	public UndergradStudent createUndergradStudent(Object[] data) throws NotPossibleException {
		...
		return new UndergradStudent(id, phone, name, address);
	}

	/**
	 * @requires
	 *           data contain id, name, phone, address, and gpa values (in that
	 *           order)
	 * @effects
	...
	public PostgradStudent createPostgradStudent(Object[] data) throws NotPossibleException {
		...
		return new PostgradStudent(id, phone, name, address, gpa);
	}
	...
	public void addStudent(Student c) {
		this.objects.add(c);
		Doc stdDoc = new Doc(c.toHtmlDoc());
		this.engine.addDoc(stdDoc);
	}

	/**
	 * @requires words != null /\ words.length > 0
	 * @effects
	 *          search for objects whose HTML documents match with the query
	 *          containing words
	...
	public Query search(String[] words) throws NotPossibleException {
		...
		result = engine.queryFirst(words[0]);
		for (int i = 1; i < words.length; i++) {
			result = engine.queryMore(words[i]);
		}
